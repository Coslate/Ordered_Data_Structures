...
const int & maxFuncCust(const int &a, const int &b){
    return (a>b?a:b);
}

int computeHeightRecur(Node *&cur_node){
    if(!cur_node){
        return -1;
    }

    cur_node->height = 1 + maxFuncCust(computeHeightRecur(cur_node->left), computeHeightRecur(cur_node->right));

    return cur_node->height;
}

void computeHeight(Node *n) {
    computeHeightRecur(n);
